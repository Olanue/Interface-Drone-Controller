...
# Inicie o visualizador de sensores se ainda não estiver rodando
def iniciar_visu_sens():
    try:
        # Caminho absoluto do script
        visu_path = os.path.join(os.path.dirname(__file__), "visu_sens.py")
        # Checa se já está rodando (simples, pode ser melhorado)
        if not os.path.exists(r'\\.\pipe\crazyflie_sensores'):
            subprocess.Popen([sys.executable, visu_path], creationflags=subprocess.CREATE_NEW_CONSOLE)
    except Exception as e:
        print(f"Não foi possível iniciar o visualizador de sensores: {e}")
...
if __name__ == '__main__':
    ...
    if os.name == 'nt':
        ...
        PIPE_NAME = '/tmp/crazyflie_sensores'
        if not os.path.exists(PIPE_NAME):
            os.mkfifo(PIPE_NAME)
    
    # Cria o pipe (servidor) – só no Windows
    if os.name == 'nt':
        try:
            pipe = win32pipe.CreateNamedPipe(
                PIPE_NAME,
                win32pipe.PIPE_ACCESS_OUTBOUND,   # só escrita
                win32pipe.PIPE_TYPE_MESSAGE | win32pipe.PIPE_WAIT,
                1, 65536, 65536,
                0,
                None
            )
            print("Pipe criado, aguardando conexão do cliente...")
            win32pipe.ConnectNamedPipe(pipe, None)  # Espera alguém conectar
            print("Cliente conectado ao pipe!")
        except pywintypes.error as e:
            print("Erro ao criar pipe:", e)
            pipe = None
    else:
        pipe = None
    
    
    def enviar_para_pipe(texto):
        try:
            if os.name == 'nt':
                win32file.WriteFile(pipe, (texto + "\n").encode())
            else:
                with open(PIPE_NAME, "w") as f:
                    f.write(texto + "\n")
        except Exception as e:
            print("Erro ao enviar dados para o pipe:", e)
    ...
    while robot.step(timestep) != -1:
        ...
        dados = f"Altura: {altitude:.2f} m | Roll: {roll:.2f} | Pitch: {pitch:.2f} | Yaw: {yaw:.2f} | Dist Frente: {range_front_value:.2f} m"
        enviar_para_pipe(dados)